...
- `sysex/cz-230s-factory/` - the 100 individual voice dumps, named with
  ...
  files (`CZ230S-00-15.syx`, etc., 16 voices per file, matching how many
  the CZ-101/CZ-1000 can hold in internal memory at once) - copied in
  unmodified (left un-renamed since each one spans a range of voices).
- All 100 are also baked directly into the in-app **Patch Library**
  ...
  the `.syx` files above with `decodeVoiceDumpMessage()`), using those
  same real factory names and tagged `factory` and `cz-230s` so they're
  easy to filter to as a group. They're deliberately *not* tagged with
  guessed mood/category words like the hand-built patches are - the real
  Casio name tells you the intended sound, but nobody's actually listened
  through all 100 yet to judge tone/mood. Give some a listen and add tags
  that fit!
